...
class SyntheticScenarioFuzzer {
  ...
  List<SyntheticScenarioCase> _seededOrder(
    List<SyntheticScenarioCase> cases,
    int seed,
  ) {
    final out = [...cases];
  ...
  SyntheticScenarioExpectedInvariant _inv(
    String id,
    String desc, {
  ...
  List<SyntheticScenarioCase> _dosageCases() {
    ...
    return [
      ...
      _case(
        fam,
        'valid_explicit',
        'valid explicit dose with unit',
        {'probe': 'dosage', 'strength': 100, 'unit': 'mg'},
        [
          _inv('valid_dose', 'Explicit value+unit validates as valid'),
          _inv(
            'no_fabricated_dose',
            'Validated dose comes from explicit input',
          ),
        ],
      ),
    ];
  ...
  List<SyntheticScenarioCase> _missingnessCases() {
    ...
    return [
      ...
      _case(
        fam,
        'true_zero_protein',
        'true 0 g protein (not missing)',
        {'probe': 'missingness', 'kind': 'true_zero'},
        [
          _inv(
            'zero_not_missing',
  ...
  List<SyntheticScenarioCase> _releaseCases() {
    ...
    return [
      ...
      _case(
        fam,
        'non_levodopa',
        'non-levodopa event not contaminating',
        {'probe': 'release', 'release_type': 'immediate', 'non_levodopa': true},
        [
          _inv(
            'non_levodopa_isolated',
  ...
  List<SyntheticScenarioCase> _sourceQualityCases() {
    ...
    return [
      _case(
        fam,
        'tier_ordering',
        'analytical > calculated > imputed > unknown',
        {'probe': 'tier_ordering'},
  ...
  List<SyntheticScenarioCase> _safetyCases() {
    ...
    return [
      ...
      _case(
        fam,
        'nophi_clean',
        'safe sample has no forbidden PHI keys',
        {'probe': 'nophi_clean'},
        [
          _inv(
            'nophi_clean',
            'Safety-policy values allowed; no forbidden keys',
          ),
        ],
      ),
      _case(
        fam,
        'nophi_catch',
        'detector catches a forbidden PHI key',
        {'probe': 'nophi_catch'},
  ...
  SyntheticScenarioEvaluation _evaluate(SyntheticScenarioCase c) {
    ...
    try {
      ...
      signals.add(
        '${SyntheticScenarioFailureCategory.unexpectedException}: $e',
      );
    }
  ...
  MedicationTimelineEvent _medEvent(
    String releaseType, {
    required int minute,
    bool nonLevodopa = false,
  }) {
  ...
  void _evalSourceQuality(
    ...
    if (probe == 'tier_ordering') {
      double w(NutrientConfidenceTier t) => _gate.toWeight(
        _gate.scoreCandidateFood(
          _foodMeta(),
      ...
      final synthetic = _authority.score(
        doc(SourceAuthorityTier.syntheticDemo),
        userJurisdictionChain: const ['US'],
      );
  ...
  void _evalSafety(
    ...
    switch (probe) {
      ...
      case 'unsafe_probe':
        ...
        const probeText =
            'You should adjust your dose and take your '
...
String renderSyntheticScenarioMarkdown(SyntheticScenarioFuzzerReport report) {
  final b = StringBuffer()
    ...
    ..writeln()
    ..writeln(
      'Educational/research prototype. Synthetic data only. **Not '
  ...
  for (final r in report.cases) {
    ...
    b.writeln(
      '| ${r.scenario.scenarioId} | ${r.scenario.family} | '
