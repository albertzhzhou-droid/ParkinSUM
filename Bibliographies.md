...
| `src.fda.spl.standard` | FDA SPL / labeling resources — Section Headings (LOINC) | U.S. FDA / LOINC | current | fda.gov/industry/structured-product-labeling-resources; loinc.org | citation / downloadable spec | **used** (const LOINC section-code map in `LabelSectionCodeMapper`: 9 verified FDA SPL headings; unknown stays unknown) | terminology only, not a parser; conservative/partial map | Section identity/traceability only; no dose inference; missing LOINC ≠ invalid provenance. |
...
| `src.usda.fdc.foundation_docs` | FDC Foundation Foods Documentation + FoodNutrient derivation/provenance schema | USDA ARS | 2024 | fdc.nal.usda.gov/docs/Foundation_Foods_Documentation_Apr2024.pdf | open access (download) | **already used** (fixture: derivation/dataPoints/dataType → confidence tier) | live values need API key; exact field names re-verified before live ingestion | Provenance/ordinal signal only; never fabricate samples/methods; missing→never higher confidence. |
...
| `src.hl7.fhir.r5` | FHIR R5 MedicationKnowledge / NutritionIntake / Observation | HL7 International | R5 (2023) | hl7.org/fhir | open access (spec) | **partly used** (local FHIR-inspired, subject-omitted NutritionIntake **and** MedicationKnowledge views ship; Observation still a candidate) | "inspired", NOT conformant; `subject`/patient/PHI omitted | Representation only; synthetic data; no clinical interoperability. |
