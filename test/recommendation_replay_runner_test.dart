...
void main() {
  test(
    'replay runner outputs deterministic ranking, AI ranking and diffs',
    () async {
      final client = MockClient((request) async {
        ...
        if ((body['format'] as Map<String, dynamic>?)?['required'] != null) {
          if ((body['messages'] as List).first['content'].toString().contains(
            'reply with {"ok":true}',
          )) {
            return http.Response(
              jsonEncode({
        ...
        final prompt = ((body['messages'] as List).first['content'] ?? '')
            .toString();
        const marker = 'Candidates JSON:\n';
        final jsonStart = prompt.indexOf(marker);
        final candidateJson = prompt
            .substring(jsonStart + marker.length)
            .trim();
      ...
      final projectionService = CdssCatalogProjectionService(
