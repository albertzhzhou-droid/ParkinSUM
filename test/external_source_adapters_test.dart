...
void main() {
  ...
  group('EU national-register importer', () {
    ...
    test(
      'parses member-state identity + register id; no SmPC → limitation',
      () {
        final r = importer.parse(
          _fixture('eu_national_register_levodopa.json'),
        );
    ...
    test('authority handled differently from EMA centralized label', () {
      ...
      expect(
        scorer.score(r.document, userJurisdictionChain: chain),
  ...
  group('registry implemented flags', () {
    test('dm+d, EU national register, NMPA all implemented now', () {
      expect(
        SourceAdapterRegistry.bySourceSystem('NHS_DMD')!.implemented,
        isTrue,
      );
      expect(
        SourceAdapterRegistry.bySourceSystem(
