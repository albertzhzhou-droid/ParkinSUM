...
void main() {
  ...
  test(
    'parses synthetic NMPA fixture into canonical metadata (no network)',
    () {
      ...
      expect(
        result.document.authorityTier,
        SourceAuthorityTier.officialLabelInJurisdiction,
      );
      expect(
        result.variant.activeIngredients,
  ...
  test('preserves reference-only translation status + sourceRefs', () {
    final result = importer.parse(_loadFixture());
    expect(
      result.variant.translationStatus,
      ReferenceTranslationStatus.referenceOnlyTranslation,
    );
    expect(
      result.document.translationStatus,
      ReferenceTranslationStatus.referenceOnlyTranslation,
    );
  ...
  test('missing active ingredient → recorded note + invalid gate', () {
    ...
    expect(
      gate.scoreMedicationContext(result.variant),
  ...
  test('missing release type lowers confidence and is noted', () {
    ...
    expect(
      result.variant.extractionConfidence!,
