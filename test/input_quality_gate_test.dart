...
void main() {
  ...
  test('unitless dose is insufficient/invalid', () {
    ...
    expect(
      [InputQualityStatus.insufficient, InputQualityStatus.invalid].contains(s),
      isTrue,
    );
  ...
  test('missing active ingredient blocks identity', () {
    ...
    expect(
      statusOf(r, InputQualityDimension.medicationIdentity),
  ...
  test('missing releaseType lowers metadata quality, not fabricated', () {
    ...
    expect(
      statusOf(r, InputQualityDimension.medicationDosage),
      InputQualityStatus.complete,
    );
  });
  ...
  test('missing sourceRefs lowers food source quality', () {
    ...
    expect(
      dim.findings.any((f) => f.findingId == 'food_missing_sourcerefs'),
      isTrue,
    );
  });
  ...
  test('empty meal is invalid', () {
    ...
    expect(
      statusOf(r, InputQualityDimension.mealComposition),
      InputQualityStatus.invalid,
    );
  ...
  test('true 0g protein is a valid zero', () {
    ...
    final dim = r.dimension(InputQualityDimension.mealComposition)!;
    expect(
      dim.findings.any((f) => f.findingId == 'meal_missing_protein'),
      isFalse,
    );
    expect(
      dim.findings.any((f) => f.findingId == 'meal_protein_true_zero'),
      isTrue,
    );
  });
  ...
  test('missing calories lowers meal completeness', () {
    ...
    expect(
      noCalDim.findings.any((f) => f.findingId == 'meal_missing_calories'),
      isTrue,
    );
  });
  ...
  test('missing portion lowers meal completeness', () {
    ...
    expect(dim.status, isNot(InputQualityStatus.complete));
    expect(
      dim.findings.any((f) => f.findingId == 'meal_missing_portion'),
      isTrue,
    );
  });
  ...
  test('synthetic source capped below official confidence', () {
    ...
    final synDim = synthetic.dimension(
      InputQualityDimension.foodSourceQuality,
    )!;
    ...
    expect(
      synDim.findings.any((f) => f.findingId == 'food_synthetic_source'),
      isTrue,
    );
  });
  ...
  test('no window blocks eligibility without advice', () {
    ...
    expect(r.mechanisticPrimaryEligible, isFalse);
    expect(
      r.fallbackReasons.contains('mechanistic_primary_window_unavailable'),
      isTrue,
    );
  ...
  test('missing localization status is info only', () {
    ...
    expect(
      dim.findings.any((f) => f.findingId == 'localization_not_provided'),
      isTrue,
    );
  });
